...
from bnb import bnb_knapsack_linear, bnb_knapsack_constant

TIMEOUT = 1800

configuracoes = [
    {"nome": "low", "input_path": "instancias/low/inputs", "opt_path": "instancias/low/labels"},
    {"nome": "large", "input_path": "instancias/large/inputs", "opt_path": "instancias/large/labels"}
]

modos = [
    ("linear", bnb_knapsack_linear),
    ("constant", bnb_knapsack_constant)
]

for modo_nome, bnb_func in modos:
    print(f"\n=== Rodando BnB modo: {modo_nome} ===")

    for config in configuracoes:
        print(f"\nProcessando instâncias: {config['nome']}")
        saida_csv = f"resultados/bnb/{config['nome']}_{modo_nome}.csv"
        ...
        with open(saida_csv, 'a', newline='') as f:
            ...
            for fname, item_csv_path, optimal_csv_path in get_file_pairs(config["input_path"], config["opt_path"]):
                ...
                tracemalloc.start()
                try:
                    result = bnb_func(items, W, timeout_seconds=TIMEOUT)
