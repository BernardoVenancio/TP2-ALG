...
for config in configuracoes:
    ...
    with open(saida_csv, 'a', newline='') as f:
        ...
        for fname, item_csv_path, optimal_csv_path in get_file_pairs(config["input_path"], config["opt_path"]):
            ...
            tracemalloc.start()
            try:
                result = knapsack_fptas(items, W, epsilon=EPSILON, timeout_seconds=TIMEOUT)
                current, peak = tracemalloc.get_traced_memory()
                tracemalloc.stop()

                status = "TIMEOUT" if result.timeout else ("OK" if result.best_value >= (1 - EPSILON) * valor_otimo else "ERRO")
                row = [
            ...
            writer.writerow(row)
            f.flush()  # Garante salvamento imediato
